...
在地址按 8 字节对齐的时候，上述算法的效率比单字节 memcpy 实现高很多，但如果地址没有按 8 字节对齐，则其效率并不高，有时甚至还比普通 memcpy 还低。这可能是因为，虽然上述算法减少了 cpu 的指令数，但内存的速度比 cpu 慢得多，速度的瓶颈还是在内存。
...
- 版本 2：使用 deque，为什么使用 deque 说的很清楚
